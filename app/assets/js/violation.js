    ...
    try {
        ...
        async function initializeData() {
            try {
                ...
                const btnExport = document.getElementById('btnExportViolations');
                ...
                function exportToPDF() {
                    try {
                        ...
                        const doc = new jsPDF();
                        const filteredViols = getFilteredViolations();
                        ...
                        const tableData = filteredViols.map(v => [
                            v.caseId || v.id,
                            v.studentName || 'N/A',
                            v.violationType || 'N/A',
                            v.date || '',
                        ...
                        doc.autoTable({
                            head: [['Case ID', 'Student', 'Violation Type', 'Date', 'Status', 'Severity']],
                            body: tableData,
                            startY: 35,
                        ...
                        doc.save(`Violations_Report_${new Date().toISOString().split('T')[0]}.pdf`);
                ...
                function exportToExcel() {
                    try {
                        const filteredViols = getFilteredViolations();
                        const data = filteredViols.map(v => ({
                            'Case ID': v.caseId || v.id,
                        ...
                        ws['!cols'] = [
                            ...
                            { wch: 15 }, { wch: 12 }, { wch: 12 }, { wch: 20 }, { wch: 40 }
                        ];
                        XLSX.utils.book_append_sheet(wb, ws, 'Violations');
                        XLSX.writeFile(wb, `Violations_Report_${new Date().toISOString().split('T')[0]}.xlsx`);
                        closeExportModalFunc();
                ...
                function exportToWord() {
                    try {
                        const filteredViols = getFilteredViolations();
                        let htmlContent = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Violations Report</title><style>
                            ...
                            })}<br>Total Violations: ${filteredViols.length}</div>
                            <table><thead><tr><th>Case ID</th><th>Student</th><th>Violation Type</th><th>Date</th><th>Status</th><th>Severity</th></tr></thead><tbody>`;

                        filteredViols.forEach(v => {
                            htmlContent += `<tr>
                                <td>${v.caseId || v.id}</td>
                                <td>${v.studentName || 'N/A'}</td>
                                <td>${v.violationType || 'N/A'}</td>
                                <td>${v.date || ''}</td>
                                <td>${v.status || 'warning'}</td>
                        ...
                        link.href = url;
                        link.download = `Violations_Report_${new Date().toISOString().split('T')[0]}.doc`;
                ...
                function exportToCSV() {
                    try {
                        const filteredViols = getFilteredViolations();
                        const headers = ['Case ID', 'Student ID', 'Student Name', 'Violation Type', 'Date', 'Status', 'Severity', 'Location', 'Description'];
                        const rows = filteredViols.map(v => [
                            v.caseId || v.id,
                            v.studentId || '',
                        ...
                        const link = document.createElement('a');
                        link.href = url;
                        link.download = `Violations_Report_${new Date().toISOString().split('T')[0]}.csv`;
                        document.body.appendChild(link);
                        link.click();
                        document.body.removeChild(link);
                        URL.revokeObjectURL(url);
                ...
                function exportToJSON() {
                    try {
                        ...
                        const jsonData = {
                            ...
                            totalViolations: filteredViols.length,
                            violations: filteredViols.map(v => ({
                                caseId: v.caseId || v.id,
                        ...
                        const link = document.createElement('a');
                        link.href = url;
                        link.download = `Violations_Report_${new Date().toISOString().split('T')[0]}.json`;
                        document.body.appendChild(link);
                        link.click();
                        document.body.removeChild(link);
                        URL.revokeObjectURL(url);
                ...
                if (btnExport) {
                    btnExport.addEventListener('click', function() {
                        if (violations.length === 0) {
                            alert('No violations to export. Please add violations first.');
                            return;
                        }
                        if (exportModal) {
                ...
                const btnImport = document.getElementById('btnImportViolations');
                ...
                function parseCSV(csvText) {
                    ...
                    const violationTypeIdx = findColumnIndex(headers, ['violation type', 'violation_type', 'type']);
                    const dateIdx = findColumnIndex(headers, ['date', 'violation date', 'date_occurred']);
                    ...
                    const locationIdx = findColumnIndex(headers, ['location', 'place']);

                    if (studentIdIdx === -1 || violationTypeIdx === -1 || dateIdx === -1) {
                        alert('Required columns not found. Please ensure the file contains "Student ID", "Violation Type", and "Date" columns.');
                        return;
                    }
                    ...
                    for (let i = 1; i < lines.length; i++) {
                        ...
                        const violation = {
                            ...
                            violationType: values[violationTypeIdx]?.trim() || '',
                            date: values[dateIdx]?.trim() || '',
                            description: descIdx >= 0 ? (values[descIdx]?.trim() || '') : '',
                            status: statusIdx >= 0 ? (values[statusIdx]?.trim().toLowerCase() || 'warning') : 'warning',
                            severity: severityIdx >= 0 ? (values[severityIdx]?.trim() || '') : '',
                            location: locationIdx >= 0 ? (values[locationIdx]?.trim() || '') : ''
                        };
                        if (violation.studentId && violation.violationType && violation.date) {
                            importData.push(violation);
                        }
                    }
                    ...
                    try {
                        ...
                        for (let i = 1; i < jsonData.length; i++) {
                            ...
                            const violation = {
                                ...
                                violationType: String(row[violationTypeIdx] || '').trim(),
                                date: String(row[dateIdx] || '').trim(),
                                description: descIdx >= 0 ? String(row[descIdx] || '').trim() : '',
                                status: statusIdx >= 0 ? String(row[statusIdx] || '').trim().toLowerCase() || 'warning' : 'warning',
                ...
                function showPreview() {
                    ...
                    let html = '<table><thead><tr><th>Student ID</th><th>Violation Type</th><th>Date</th><th>Status</th></tr></thead><tbody>';
                    previewRows.forEach(row => {
                        html += `<tr><td>${row.studentId}</td><td>${row.violationType}</td><td>${row.date}</td><td>${row.status}</td></tr>`;
                    });
                    html += '</tbody></table>';
                ...
                if (confirmImportBtn) {
                    confirmImportBtn.addEventListener('click', async function() {
                        ...
                        try {
                            ...
                            formData.append('violations', JSON.stringify(importData));
                            const response = await fetch('../api/violations.php?action=import', {
                                method: 'POST',
                                body: formData
                            });
                            const result = await response.json();
                            if (result.status === 'success') {
                                alert(`Successfully imported ${result.data.imported || importData.length} violation(s)!`);
                                closeImportModalFunc();
                                initializeData(); // Reload violations
                            } else {
                            ...
                            alert('Error importing violations. Please try again.');
                            this.disabled = false;
                            this.classList.remove('loading');
                            this.innerHTML = originalText;
                        }
